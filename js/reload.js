...
upEventEles.forEach((upEveEle) => {
  //すべてのid=js-upeventから始まるものに以下のものを実行

  let upPageNum; //この宣言は移動不可

  switch (
    upEveEle.id.length //page数を取得
  ) {
    case 11:
    ...
    default:
      upPageNum =
        parseInt(upEveEle.id[10], 10) * 10 + parseInt(upEveEle.id[11], 10);
  }
  ...
  //「詳細」を押した際のイベント
  document
    .getElementById("js-upBalButLeft" + upPageNum)
  ...
  //お気に入りから削除できるようにする
  document
    .getElementById("js-upBalButRight" + upPageNum)
    .addEventListener("click", () => {
      hide(upPageNum);
      check[upPageNum] = 0;
